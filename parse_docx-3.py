...
class DocxParser:
    ...
    def parse_p(self, p):
        ...
        for child in children:
            ...
            tag_namespace = etree.QName(child).namespace
            quan_child_tag = self.namespaces_dict.get(tag_namespace) + child_tag
            if child_tag == "pPr":
                continue
            if child.xpath(".//w:p", namespaces=self.namespaces):
                r_dict = xml2dict.parse(etree.tostring(child, encoding="utf-8").decode("utf-8"),
                                        strip_whitespace=False)
                r_dict["tag"] = quan_child_tag
            ...
            # 遇到oMath标签，要拆句
                ...
                for r in r_list:
                    char_begin = r.xpath('.//w:fldChar[@w:fldCharType="begin"]', namespaces=self.namespaces)
                    if char_begin:
                        text_list.append("".join(one_text_list))
                        one_text_list = []
                    r_dict = xml2dict.parse(etree.tostring(child, encoding="utf-8").decode("utf-8"),
                                            strip_whitespace=False)
                    r_dict["parent"] = "hyperlink"
                    r_dict["tag"] = quan_child_tag
                    rd_list.append(r_dict)
            elif child_tag == "r" and child.xpath('.//w:t', namespaces=self.namespaces):
    ...
    def join_xml(self, p_node, p_infos, p_info_index, p_namespaces_dict, tree):
        ...
        # 组装新的p
        for sentence_r_dict in sentence_rs:
            tag = sentence_r_dict["tag"]
            xml_dict = sentence_r_dict[tag]
            xml_dict.update(p_namespaces_dict)
            xml_root_dict = {tag: xml_dict}
            # sentence_r_dict[tag].update(p_namespaces_dict)
            # sentence_r_dict["w:r"].update(xml_root_dict)
            r_xml_str = xml2dict.unparse(xml_root_dict)
            r_xml_str = r_xml_str.replace("""<?xml version="1.0" encoding="utf-8"?>""", '').strip()
            trans_p_node.append(etree.fromstring(r_xml_str))
